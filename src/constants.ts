...
Требования:
1) Ответ должен быть строго в JSON, без текста вне JSON.
2) Все задания и объяснения — на английском языке.
3) Генерируй упражнения так, чтобы их можно было проверять без LLM:
   - ответы должны быть однозначными
   - для каждого задания дай список допустимых правильных ответов (accepted_answers)
4) Типы упражнений в пакете (минимум 2 типа):
   - fill_blank (вставь слово/форму)
   - choose_one (выбери правильный вариант)
   - rewrite (перепиши предложение по правилу) — только если можно дать 2–4 допустимых ответа
5) Количество упражнений: {{COUNT}} (обычно 8–12)
6) Сложность под уровень пользователя: {{LEVEL}}
7) Не используй редкую лексику. Темы — повседневные или рабочие (для разработчика).
8) Для каждого задания дай:
   - id
   - type
   - prompt
   - choices (только для choose_one)
   - accepted_answers (массив строк)
   - short_explanation (1–2 предложения)
9) Не добавляй HTML/форматирование — это будет делаться отдельно.
...
JSON-формат:
{
  ...
  "items": [
    {
      "id": "ex_01",
      "type": "fill_blank | choose_one | rewrite",
      "prompt": "...",
      "choices": ["..."] ,              // только для choose_one
      "accepted_answers": ["..."],
      "short_explanation": "..."
